...
class TestBioPlateArray(unittest.TestCase):
    ...
    def test_merge_stack(self):
        id1 = id(self.stack1)
        id2 = id(self.stack2)
        self.assertEqual(BioPlateArray._merge_stack(id1, id2), [id(self.plate2), id(self.plate3),
                                                                id(self.plate4), id(self.plate5)])
    ...
    def test_get_list_id_of_stack(self):
        with self.assertRaises(ValueError):
            self.assertEqual(BioPlateArray._get_list_id_of_stack(self.plate2), [id(self.plate2),])
        self.assertEqual(BioPlateArray._get_list_id_of_stack(self.stack1), [id(self.plate2), id(self.plate3)])
